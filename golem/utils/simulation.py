...
class IIDSimulation(object):
    ...
    def __init__(self, W, n=1000, method='linear', 
        ...
        print('Finished synthetic dataset')
...
def main():
    ...
    dag_simulator = DAG()
    W = dag_simulator.erdos_renyi(
            n_nodes=config["d"],
            n_edges=config["s0"],
            weight_range=(0.5, 2.0)
    )
    print("W shape:", W.shape)
    assert W.shape == (config["d"], config["d"])
    print("W shape test pass!")
    ...
    assert X.shape == (config["n"], config["d"])
    print("X shape test pass!")
#%%
